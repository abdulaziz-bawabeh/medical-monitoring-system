...
async function startServer() {
  try {
    ...
    console.log(`Database user: ${databaseInfo.database_user}`);
    ...
    httpServer.listen(PORT, () => {
      console.log("Medical Monitoring Backend");
      console.log(`HTTP and Socket.IO: http://localhost:${PORT}`);
...
async function shutdown(signal) {
  ...
  httpServer.close(async (error) => {
    ...
    console.log("HTTP server, Socket.IO and PostgreSQL pool closed.");
    process.exit(0);
  });
}
